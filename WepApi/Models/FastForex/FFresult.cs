...
public class FFresult : ModelBase
{
    private Dictionary<string, decimal>? dictonCurrencies = null;
    private Dictionary<string, decimal> GetCourses() =>
        this.GetType()
            .GetProperties(BindingFlags.Instance | BindingFlags.Public)
            .Where(prop => prop.Name != "ID")
            .ToDictionary(prop => prop.Name, prop => decimal.Parse(prop.GetValue(this, null).ToString()));

    internal decimal this[string currency]
    {
        get
        {
            if (dictonCurrencies is null)
                dictonCurrencies = GetCourses();

            return dictonCurrencies[currency];
        }
    }
