<script>
    ...
    const ActionToRemove = async ({id}) => {
        try {
            await Request.transaction.delete({BudgetID: budgetID, TransactionID: id});
...
{#each groupedTransactions as groupedTransaction}
    <div class="pr-4 mb-5 w-1/2 bg-white rounded-lg border shadow-md sm:p-8">
        <div class="flex justify-between items-center mb-4">
            <h5 class="text-xl font-bold leading-none text-gray-900">{new Date(groupedTransaction.date).toDateString()}</h5>
        </div>
        <div class="flow-root">
            <ul role="list" class="divide-y divide-gray-200">
                {#each groupedTransaction.transactions as transaction}
                    <Dialog ModalID="modal_remove_t_{transaction.id}" ConfirmFunction={ActionToRemove}
                        ...
                        <div class="flex">
                            <div class="flex-0 min-w-0 ">
                                ...
                                <p class="text-sm text-gray-500 truncate">
                                    {new Date(transaction.date).toLocaleString().split(",")[1]}
                                </p>
                            </div>
                                    ...
                                    {#if transaction.notes.length > 20}
                                        <div tabindex="0" class="collapse">
                                            <div class="collapse-title">
                                                <p class="text-sm text-gray-600 truncate"> {transaction.notes.substring(0, 20)}
                                                    ...</p>
                                            </div>
                                            <div class="collapse-content">
                                                <p class="text-sm text-gray-600 truncate"> {transaction.notes}</p>
                                            </div>
                                        </div>
                                    {:else}
                                        <div class="collapse-title">
