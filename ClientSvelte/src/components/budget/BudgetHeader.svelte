    ...
    onMount(async () => {
        location.subscribe(async (newLocation) => {
            if (newLocation.includes("budget")) {
                ...
                try {
                    $selectedBudget = (await Request.budget.get({ BudgetID }))
                        .data;
                } catch (err) {
                    ErrorWrapper(err);
                }
...
<div class="navbar bg-yellow-100">
    <div class="navbar-start">
        <a
            ...
            href="/#/budget/{$selectedBudget.id}"
        >
            <b>{$selectedBudget.name || "Budget"}</b>
        </a>
        <a
            class="btn btn-ghost normal-case"
            href="/#/budget/{$selectedBudget.id}/statistic/"
        >
            Statistic
        </a>
    </div>
    <div class="navbar-center">
        {#if $selectedBudget.balance?.amount > 0}
    ...
    <div class="navbar-end">
        <a
            class="btn btn-ghost normal-case"
            href="/#/budget/{$selectedBudget.id}/assistant"
        >
            Assistant
        </a>
        <a
            class="btn btn-ghost normal-case"
            href="/#/budget/{$selectedBudget.id}/planned/"
        >
            Planned
        </a>
        <a
            class="btn btn-ghost normal-case"
            href="/#/budget/{$selectedBudget.id}/category/"
        >
            Categories
        </a>
    </div>
